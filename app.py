...
st.set_page_config(page_title="FLUX AI - 優化端點版", page_icon="🌐", layout="wide")
...
def show_api_settings():
    ...
    with st.expander("📝 編輯存檔內容", expanded=True):
        ...
        sel_prov_name = st.selectbox("API 提供商", provs, index=provs.index(active_config.get('provider', 'NavyAI')), format_func=lambda x: f"{API_PROVIDERS[x]['icon']} {API_PROVIDERS[x]['name']}")
        
        # 當提供商改變時，更新 URL 為新的預設值
...
st.markdown("""<div style="text-align: center; color: #888; margin-top: 2rem;"><small>🌐 優化端點版 | 部署在 Koyeb 免費實例 🌐</small></div>""", unsafe_allow_html=True)
