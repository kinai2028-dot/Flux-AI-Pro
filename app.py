...
# 風格預設
STYLE_PRESETS = {
    "無": "", "電影感": "cinematic, dramatic lighting, high detail", "動漫風": "anime style, vibrant colors",
    "賽博龐克": "cyberpunk, neon lights", "水彩畫": "watercolor painting", "奇幻藝術": "fantasy art, epic, magical",
}
...
st.set_page_config(page_title="FLUX Krea AI Studio", page_icon="🎨", layout="wide")
...
def init_session_state():
    if 'api_profiles' not in st.session_state:
        st.session_state.api_profiles = {
            "預設 Krea": {'provider': 'Krea AI Studio', 'api_key': '', 'base_url': 'https://api.krea.ai/v1', 'validated': False}
        }
...
def generate_images_with_retry(client, **params) -> Tuple[bool, any]:
    ...
    # 移除不適用於所有 API 的參數
    params.pop("acceleration", None)

    try:
        if provider == "Krea AI Studio":
            # Krea 特定的參數
            krea_params = params.copy()
            krea_params["prompt"] = prompt
            # 此處應根據 Krea API 文檔加入加速模式等參數
            # krea_params["acceleration"] = params.get("acceleration", "none")
            return True, client.images.generate(**krea_params)
        # ... 其他提供商的邏輯 ...
        else:
...
def show_api_settings():
    ...
    with st.expander("📝 編輯存檔內容", expanded=True):
        ...
        sel_prov = st.selectbox("API 提供商", provs, index=provs.index(active_config.get('provider', 'Krea AI Studio')), format_func=lambda x: f"{API_PROVIDERS[x]['icon']} {API_PROVIDERS[x]['name']}")
...
st.title("🎨 FLUX Krea AI Studio - 專業美學版")
...
with tab1:
    if not api_configured: st.warning("⚠️ 請在側邊欄選擇一個已驗證的存檔。")
        ...
        sel_model = st.selectbox("模型:", ["flux.1-krea-pro", "flux.1-krea-dev"]) # 假設的模型
        ...
        # Krea AI 專用選項
        acceleration = "none"
        if cfg.get('provider') == "Krea AI Studio":
            with st.expander("🎨 Krea AI 專用選項", expanded=True):
                acceleration = st.select_slider(
                    "創意加速模式",
                    options=["none", "regular", "fast"],
                    format_func=lambda x: {"none": "無", "regular": "常規 (快20%)", "fast": "高速 (快50%)"}[x]
                )
        ...
        if st.button("🚀 生成圖像", type="primary", use_container_width=True, disabled=not prompt_val.strip()):
            ...
            with st.spinner(f"🎨 正在生成圖像..."):
                params = {"model": sel_model, "prompt": final_prompt, "negative_prompt": negative_prompt_val, "size": final_size_str, "acceleration": acceleration}
