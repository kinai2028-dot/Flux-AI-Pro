...
st.set_page_config(page_title="Flux AI (最終版)", page_icon="🏆", layout="wide")
...
def generate_images_with_retry(client, **params) -> Tuple[bool, any]:
    ...
    for attempt in range(3):
        try:
            if provider == "Pollinations.ai":
                api_params = {k: v for k, v in {"model": params.get("model"), "width": params.get("size", "1024x1024").split('x')[0], "height": params.get("size", "1024x1024").split('x')[1], "seed": random.randint(0, 1000000), "nologo": params.get("nologo"), "private": params.get("private"), "enhance": params.get("enhance")}.items() if v is not None}
...
with tab1:
    if not api_configured:
        ...
        if not all_models:
            ...
            with col2: num_images = 1 if cfg['provider'] == "Pollinations.ai" else st.slider("生成數量", 1, MAX_BATCH_SIZE, 1)
...
st.markdown("""<div style="text-align: center; color: #888; margin-top: 2rem;"><small>🏆 最終整合版 | 部署在 Koyeb 免費實例 🏆</small></div>""", unsafe_allow_html=True)
